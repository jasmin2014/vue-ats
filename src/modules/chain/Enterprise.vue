  ...
  <div class="enterprise">
    ...
    <el-form ref="baseForm"
      ...
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="联系电话" prop="contactPhone" :error="error.contactPhone">
    ...
    <el-form ref="legalForm"
      ...
      <el-row>
        <el-col :span="8">
          <el-form-item label="电话号码" prop="workOrgPhone">
            <ats-input v-model.trim="currentValue.legal.workOrgPhone"
                       :mode="mode"></ats-input>
          </el-form-item>
        </el-col>
        <el-col :span="8">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      file1: {
        get() {
          return this.currentValue.materialList.filter(_ => _.kind === this.$enum.PROOF_MATERIAL_O_LICENSE).map((_, index) => ({
            id: _.id || index,
            name: _.name,
            url: _.fileUri,
            type: _.fileType
    ...
    methods: {
      save() {
        ...
        this.$refs.baseForm.validate((valid) => {
          if (valid) {
            this.$refs.businessForm.validate((v) => {
              if (v) {
                this.$refs.legalForm.validate((_v) => {
                  if (this.validate(val) && _v) {
                    this.emitSave(val)
                  } else if (!_v) {
                    this.$emit('error');
      ...
      handleRemoveLicense(file) {
        this.currentValue.materialList = this.currentValue.materialList.filter(_ => _.id !== file.id)
      }
    },
    components: {
      LegalChoose
