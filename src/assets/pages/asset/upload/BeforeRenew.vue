  ...
  <div class="upload">
    <el-row type="flex" justify="center">
      <el-col :span="10">
        <el-form>
          <el-form-item>
            <div class="upload-input" style="text-align: center;">
              <a href="/static/doc/录入续借参数模板.zip">
                <el-button>续借模板</el-button>
              </a>
            </div>
          </el-form-item>
...
<script>
  ...
  export default {
    data() {
      return {
        formData: {
          type: this.$enum.LOAN_PROP_CONTINUE
        },
        error: {
          type: '',
          upload: ''
    ...
    methods: {
      ...
      handleUploadList(row) {
        this.$confirm('确定上传所有资产数据?', '提示', {
          ...
          uploadRenewZip(row.id).then(({data}) => {
            this.getData(this.search.pageNumber);
            if (data.code !== 200) {
      ...
      handleDelete(row) {
        this.$confirm('确定删除?', '提示', {
          ...
          deleteRenewZip(row.id).then(({status}) => {
            if (status === 204) {
              this.getData(this.search.pageNumber);
              this.$message({
                message: '删除成功',
