  ...
  <div>
    <el-form class="form-label flex fixed-width">
      <el-row>
        ...
        <el-col :span="8">
          ...
          <el-button v-action="'AssetPublicDownload'"
                     type="warning" icon="fa fa-download" title="导出数据到Excel"
                     @click="handleDownloadExcel"></el-button>
          <el-button v-action="'AssetPublicBatchStore'"
                     type="success" icon="fa fa-book" title="批量收纳"
                     @click="handleStoreBatch()">&nbsp;批量收纳</el-button>
          <el-button v-action="'AssetPublicSetting'"
                     type="info" icon="fa fa-cog" title="准入设置"
                     @click="handleSetting">&nbsp;准入设置</el-button>
        </el-col>
      </el-row>
    </el-form>
    ...
    <el-row>
      <el-table :data="list" border
        ...
        <el-table-column v-if="$store.state.buttons['AssetPublicDetail'] || $store.state.buttons['AssetPublicStore']"
                         label="操作" align="center" fixed="right" width="120">
          <template slot-scope="scope">
            <el-tooltip v-action="'AssetPublicDetail'"
                        content="查看">
              <el-button size="small" icon="fa fa-eye"
                         @click="handleDetail(scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip v-action="'AssetPublicStore'"
                        content="收纳">
...
<script>
  import {getAssetList, getLoanRemark, storeAsset, downloadLoanApplicationList} from '../../../api/assetMgt';
