  ...
  <div class="stat-list">
    <el-form label-width="90px">
      ...
      <el-form-item label="资产渠道：">
        <el-checkbox v-model="assetOrgCheckAll" :indeterminate="search.assetOrgIds.length > 0 && search.assetOrgIds.length < assetOrgOptions.length">全选</el-checkbox>
        <el-checkbox-group v-model="search.assetOrgIds">
          <el-checkbox v-for="option in assetOrgOptions"
                       :key="option.orgId"
...
<script>
  ...
  import { getAssetListFromFund } from '../../../api/enum'
  ...
  export default {
    ...
    data() {
      return {
        ...
        chartBind: {
          loading: false,
          height: '400px',
          title: { text: '资产各期限份额占比图', left: '240px', textStyle: { fontSize: 22 } },
    ...
    computed: {
      ...
      assetOrgCheckAll: {
        ...
        set(val) {
          this.search.assetOrgIds = val ? this.assetOrgOptions.map(_ => _.orgId) : [];
        }
      },
    ...
    methods: {
      ...
      getAssetOrgList() {
        ...
        getAssetListFromFund(orgId).then(({ data }) => {
          if (data.code === 200) {
            this.assetOrgOptions = data.body
          }
        })
      }
    }
