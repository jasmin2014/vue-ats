...
<script>
  ...
  import {getFinishDetail} from '../../api/asset';
  ...
  export default {
    ...
    methods: {
      getDetail() {
        getFinishDetail(this.id).then(({ data }) => {
          if (data.code === 200) {
            const customer = {
            ...
            const result = {
              loanLimit: data.body.loanApplicationDTO.loanLimit,
              shareLimit: data.body.loanApplicationDTO.shareLimit,
              sharedRateLowerLimit: data.body.loanApplicationDTO.sharedRateLowerLimit,
              sharedRateUpLimit: data.body.loanApplicationDTO.sharedRateUpLimit,
              loanShareConfirms: data.body.loanShareConfirms
            };
            this.$set(this.detail, 'customer', customer);
            this.$set(this.detail, 'apply', apply);
            this.$set(this.detail, 'result', result);
          } else {
